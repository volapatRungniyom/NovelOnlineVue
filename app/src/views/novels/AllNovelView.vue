<template>
  ...
  <div v-if="novels == null" role="status">
    ...
    <div v-for="novel in novels"
         :key = "novel.id"
         class="p-4 mb-d border-2 border-blue-800 rounded-lg m-5 cursor-pointer" @click="ShowNovel(novel)"
    >
        <button @click="ShowNovel(novel)">
          pics
        </button>
        <div class="text-3xl">
          {{ novel.name}}
        ...
        <div v-if="novel.tags" class="well">
          tag
          <span v-for=" tag in novel.tags" :key = "tag.id" class="badge badge-primary mr-2">
        #{{ tag.name }}
      </span>
        </div>
...
export default {
  ...
  async mounted() {
    const url = "http://localhost/api/novels"
    try {
